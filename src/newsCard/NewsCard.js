...
const NewsCard = ({news}) => {
    const {_id,author,details,thumbnail_url,title,rating,total_view} = news;
    return (
	...
	<div>
		...
		<p className="text-sm dark:text-gray-400">{details.length===200?details:`${details.slice(1,200)}....`} <Link to={`/categoryById/${_id}`} className='text-blue-800 font-bold underline'>More</Link></p>
	</div>
